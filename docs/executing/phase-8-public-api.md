...
**Goal:** Make Lumber importable as a Go library. After this phase, any Go application can `go get github.com/crimson-sun/lumber` and classify log text into canonical events — no subprocess, no stdout parsing, no CLI.
...
Currently the module is `github.com/crimson-sun/lumber`. The public API would be imported as:

```go
import "github.com/crimson-sun/lumber/pkg/lumber"
```
